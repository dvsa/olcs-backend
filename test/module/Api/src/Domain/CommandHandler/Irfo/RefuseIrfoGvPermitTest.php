...
use Dvsa\Olcs\Api\Domain\CommandHandler\Irfo\RefuseIrfoGvPermit as Sut;
...
class RefuseIrfoGvPermitTest extends CommandHandlerTestCase
{
    ...
    {
        $this->refData = [
            IrfoGvPermitEntity::STATUS_REFUSED,
        ];
    ...
    public function testHandleCommand()
    {
        ...
        $irfoGvPermit->shouldReceive('refuse')
            ->once()
            ->with($this->refData[IrfoGvPermitEntity::STATUS_REFUSED])
            ->shouldReceive('getId')
            ->andReturn($id);
