...
final class ProcessEcmtApplications extends AbstractCommandHandler implements TransactionedInterface
{
    ...
    public function handleCommand(CommandInterface $command)
    {
        ...
        foreach ($ecmtPermitApplications as $ecmtPermitApplication) {
            ...
            if ($candidatePermitStatus['successful'] > 0) {
                $result->merge(
                    $this->handleSideEffect(
                        $this->getApplicationFeeCommand($ecmtPermitApplication)
                    )
                );
            ...
            $ecmtPermitApplication->setStatus($awaitingFeeStatus);
    ...
    /**
     * Get application fee command for an application
     *
     * @param EcmtPermitApplication $ecmtPermitApplication
     *
     * @return FeeEntity
     */
    private function getApplicationFeeCommand(EcmtPermitApplication $ecmtPermitApplication)
    {
        $feeType = $this->getRepo('FeeType')->getLatestForEcmtPermit(self::APPLICATION_FEE_PRODUCT_REFERENCE);
