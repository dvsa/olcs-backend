    ...
    private function updateAuthService(User $newUser, ?string $previousEmailAddress, bool $previouslyDisabled): void
    {
        ...
        if ($provider === JWTIdentityProvider::class) {
            if ($newUser->getContactDetails()->getEmailAddress() !== $previousEmailAddress) {
                $this->adapter->changeAttribute($newUser->getLoginId(), 'email', $newUser->getContactDetails()->getEmailAddress());
            }

            if ($newUser->isDisabled() && !$previouslyDisabled) {
                $this->adapter->disableUser($newUser->getLoginId());
            } elseif (!$newUser->isDisabled() && $previouslyDisabled) {
                $this->adapter->enableUser($newUser->getLoginId());
            }
        } else {
