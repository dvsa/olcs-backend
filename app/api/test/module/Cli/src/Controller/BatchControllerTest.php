...
class BatchControllerTest extends MockeryTestCase
{
    ...
    public function testIdentifySuccessfulPermitApplicationsAction()
    {
        $stockId = 7;

        $this->pm->shouldReceive('get')
            ->with('params', null)
            ->andReturn($stockId);
        ...
        $this->mockQueryHandler->shouldReceive('handleQuery')
            ->with(
                equalTo(
                    CliQuery\Permits\StockAvailability::create(
                        ['stockId' => $stockId]
                    )
                )
            )
        ...
        $this->mockQueryHandler->shouldReceive('handleQuery')
            ->with(
                equalTo(
                    CliQuery\Permits\StockLackingRandomisedScore::create(
                        ['stockId' => $stockId]
                    )
                )
            )
        ...
        $this->mockCommandHandler->shouldReceive('handleCommand')
            ->with(
                equalTo(
                    CliCommand\Permits\MarkSuccessfulSectorPermitApplications::create(
                        ['stockId' => $stockId]
                    )
                )
            )
        ...
        $this->mockCommandHandler->shouldReceive('handleCommand')
            ->with(
                equalTo(
                    CliCommand\Permits\MarkSuccessfulDaPermitApplications::create(
                        ['stockId' => $stockId]
                    )
                )
            )
        ...
        $this->mockCommandHandler->shouldReceive('handleCommand')
            ->with(
                equalTo(
                    CliCommand\Permits\MarkSuccessfulRemainingPermitApplications::create(
                        ['stockId' => $stockId]
                    )
                )
            )
    ...
    public function testIdentifySuccessfulPermitApplicationsActionInvalidStockId()
    {
        $stockId = 'test';
    ...
    public function testIdentifySuccessfulPermitApplicationsActionStockAvailabilityFailed()
    {
        $stockId = 7;
