...
CREATE TABLE `person` (
  ...
  `version` smallint(5) unsigned NOT NULL DEFAULT '1' COMMENT 'Optimistic Locking',
  `olbs_key` int(10) unsigned DEFAULT NULL COMMENT 'Used to map FKs during ETL. Can be dropped safely when OLBS decommissioned',
  `olbs_type` varchar(32) DEFAULT NULL COMMENT 'used to differentiate source of data during ETL when one OLCS table relates to many OLBS. Can be dropped when fully live',
  PRIMARY KEY (`id`),
